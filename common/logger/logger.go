...
func init() {
	...
	if os.Getenv("BAZEL_TEST") == "1" {
		...
		level = DebugLevel
	} else if os.Getenv("ASPECT_LOG_FILE") != "" {
		logfile, err := os.Create(os.Getenv("ASPECT_LOG_FILE"))
		if err != nil {
	...
	if os.Getenv("ASPECT_LOG_LEVEL") != "" {
		envLevel := strings.ToUpper(strings.TrimSpace(os.Getenv("ASPECT_LOG_LEVEL")))
		switch envLevel {
		case "TRACE":
			level = TraceLevel
		case "DEBUG":
