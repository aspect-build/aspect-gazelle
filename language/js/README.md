...
| An extension of the standard `map_kind` directive: appending `:group` to the source kind restricts the mapping to targets generated from that source group. For example, the following maps only the test targets:<br />`# gazelle:map_kind ts_project:{dirname}_tests jest_ts_project //tools:jest.bzl`<br /><br />**Group name.** The group is named the same way as in `js_files`/`js_test_files` — its configured target name: `{dirname}` and `{dirname}_tests` for the default library and tests groups (or their `js_project_naming_convention`/`js_tests_naming_convention` overrides), or the custom target name. `ts_project:<group>` is the group key: the mapping applies to the entire group, including packages where the group has no transpiled sources and its target is generated as `js_library`.<br /><br />**`alias_kind`.** A scoped mapping needs an `alias_kind` registration declaring which kind the new macro wraps, so that Gazelle merges, resolves, and indexes existing rules of the macro kind. When none is declared it is defaulted automatically: the macro is registered as wrapping the source kind — `ts_project`, or the kind a plain `map_kind` of `ts_project` maps it to (e.g. the default for the example above is `alias_kind jest_ts_project ts_project`). Declaring it explicitly (the standard directive) is still recommended when other Gazelle-based tooling walks the same BUILD files without this extension, since only the declared form is visible to it. The registered alias must name the kind the macro wraps in each directory — when a subtree's plain `map_kind` changes the wrapped kind under an inherited scoped mapping, declare the `alias_kind` explicitly there. A mismatch is a configuration error that aborts generation, as is mapping to a built-in kind or using a `js_library:<group>` key instead of `ts_project:<group>`.<br /><br />**The macro.** It must instantiate the wrapped kind under the macro call's own name and forward attributes such as `srcs` and `deps` to it — Gazelle resolves dependencies to the macro's label expecting the wrapped kind's providers there.<br /><br />**Scope.** Like `map_kind`, scoped mappings may be declared in any directory, apply to the declaring directory and its descendants, and may be overridden in subdirectories. An inherited mapping keeps applying when a subdirectory renames the group (e.g. via `js_tests_naming_convention`): the group named in the directive is resolved under the naming convention of the declaring directory, and the declaration nearest to each directory wins. A `:group` naming no configured group is reported as a warning and otherwise ignored; because only directories visited in the run are known, this warning may appear for a group defined outside the visited set when running Gazelle on a subset of directories. |
