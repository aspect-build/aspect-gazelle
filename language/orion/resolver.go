...
func (host *GazelleHost) resolveImport(
	...
	// One spec for ordinary imports, or one per ancestor dir for Ancestor imports.
	for _, spec := range importSpecsToTry(impt, importSpec, from.Pkg) {
		matches := ix.FindRulesByImportWithConfig(c, spec, GazelleLanguageName)
		if len(matches) == 0 {
			continue
		}
		...
		if len(filtered) > 1 {
			return Resolution_Error, nil, fmt.Errorf(
				...
				spec.Imp, impt.From, pluginId, targetListFromResults(matches))
		}
	...
	// Ancestor imports have no symbol-DB fallback.
	if impt.Ancestor {
...
// importSpecsToTry yields `base` for ordinary imports, or `impt.Id` prefixed with each ancestor of `fromPkg` for Ancestor imports.
func importSpecsToTry(impt plugin.TargetImport, base resolve.ImportSpec, fromPkg string) []resolve.ImportSpec {
	if !impt.Ancestor {
		return []resolve.ImportSpec{base}
	}
	var specs []resolve.ImportSpec
	rel := fromPkg
	for {
		spec := resolve.ImportSpec{Lang: base.Lang, Imp: impt.Id}
		if rel != "" {
			spec.Imp = rel + "/" + impt.Id
		}
