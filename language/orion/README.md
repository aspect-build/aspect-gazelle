...
See a basic [rules_cc example](./tests/starzelle/rules-cc/rules_cc.axl)
...
> **Do not register a kind that another enabled gazelle language already
> provides** (e.g. `js_library`, `ts_project`, `go_library`). The runner
> aborts with an error in that case, since gazelle's last-wins behavior
> would otherwise silently clobber the other language's rule resolution.
...
*Args*:
- `id`: a unique identifier for the extension, may be referenced in Starlark API or used in `# gazelle:{name} enabled|disabled` directives etc
...
* `.add(name, kind[, attrs][, symbols])`: add a rule of the specified kind to the `BUILD` file with a set of attributes and exported symbols
  ...
  * `kind`: the rule kind, a native/builtin rule or one registered with `aspect.gazelle_rule_kind`
