"""Always-opt rust_static_library wrapper for the cgo-linked tree-sitter bridge.

`with_cfg` pins compilation_mode=opt on this target's subtree -- the crate plus
the tree-sitter grammar deps where parse time actually goes -- so the bridge is
built at opt-level 3 no matter the consumer's build mode. That matters when
aspect_gazelle_orion is used as a source bazel_dep inside someone's
