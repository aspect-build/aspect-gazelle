...
> If you use [\`aspect_gazelle_prebuilt\`](https://github.com/aspect-build/aspect-gazelle/tree/main/prebuilt), its functionality is already included in the prebuilt binary and no other \`aspect_gazelle_*\` module is required.
...
cat <<EOF
> [!NOTE]
> \`aspect_gazelle_prebuilt\` is the only module you need — every language and extension is already compiled into the prebuilt binary.
> The other \`aspect_gazelle_*\` modules (\`aspect_gazelle_js\`, \`aspect_gazelle_kotlin\`, \`aspect_gazelle_orion\`, \`aspect_gazelle_runner\`, ...) are only used when building the Gazelle binary from source.
